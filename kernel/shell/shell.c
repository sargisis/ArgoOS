...
static int cmd_test_sleep(int argc, char **argv);
...
void shell_init(void) {
  ...
  shell_register_command("test_sync", "Test synchronization primitives",
                         cmd_test_sync);
  shell_register_command("test_sleep", "Test timer-based sleep",
                         cmd_test_sleep);
}
...
static void sync_task1() {
  serial_puts("### sync_task1 STARTED ###\n");
  for (int i = 0; i < 5; i++) {
    mutex_lock(&test_mutex);
    int val = shared_counter;
    task_sleep(100); // Sleep while holding lock to force contention
    shared_counter = val + 1;
    serial_puts("[Task 1] incremented counter to: ");
    serial_putdec(shared_counter);
    serial_puts("\n");
    mutex_unlock(&test_mutex);
    task_sleep(50);
  }
  serial_puts("### sync_task1 EXITED ###\n");
  task_exit();
}

static void sync_task2() {
  serial_puts("### sync_task2 STARTED ###\n");
  for (int i = 0; i < 5; i++) {
    mutex_lock(&test_mutex);
    int val = shared_counter;
    task_sleep(100);
    shared_counter = val + 1;
    serial_puts("[Task 2] incremented counter to: ");
    serial_putdec(shared_counter);
    serial_puts("\n");
    mutex_unlock(&test_mutex);
    task_sleep(50);
  }
  serial_puts("### sync_task2 EXITED ###\n");
  task_exit();
}
...
static int cmd_test_sync(int argc, char **argv) {
  ...
  shared_counter = 0;
  serial_puts("Starting synchronization test...\n");
  task_create(sync_task1, "sync_task1", 1);
  task_create(sync_task2, "sync_task2", 1);
  return 0;
}
...
static int cmd_test_sleep(int argc, char **argv) {
  (void)argc;
  (void)argv;
  serial_puts("Sleeping for 1 second...\n");
  task_sleep(1000);
  serial_puts("Awake!\n");
  return 0;
}
...
static int cmd_tail(int argc, char **argv) {
  ...
  uint32_t lines_to_skip =
      (total_lines > num_lines) ? (total_lines - num_lines) : 0;
